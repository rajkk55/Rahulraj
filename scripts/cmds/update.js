...
module.exports = {
	...
	langs: {
		vi: {
			...
			updatePrompt: "💫 | Bạn đang sử dụng phiên bản %1. Hiện tại đã có phiên bản %2. Bạn có muốn cập nhật chatbot lên phiên bản mới nhất không?"
				...
				+ "\n%3%4"
				+ "\n\nℹ️ | Xem chi tiết tại https://github.com/rajkk55/Rahulraj/commits/main"
		...
		en: {
			...
			updatePrompt: "💫 | You are using version %1. There is a new version %2. Do you want to update the chatbot to the latest version?"
				...
				+ "\n%3%4"
				+ "\n\nℹ️ | See details at https://github.com/rajkk55/Rahulraj/commits/main"
	...
	onStart: async function ({ message, getLang, commandName, event }) {
		// Check for updates
		const { data: { version } } = await axios.get("https://raw.githubusercontent.com/rajkk55/Rahulraj/main/package.json");
		const { data: versions } = await axios.get("https://raw.githubusercontent.com/rajkk55/Rahulraj/main/versions.json");
	...
	onReaction: async function ({ message, getLang, Reaction, event, commandName }) {
		...
		const { data: lastCommit } = await axios.get('https://api.github.com/repos/rajkk55/Rahulraj/commits/main');
